...
void paste(vector<vector<char>> &p1, pair<int,int> pos1,vector<vector<char>> &p2, pair<int,int> pos2,vector<vector<char>> &p3, pair<int,int> pos3){
  ...
  for(int i=0;i<4;i++){
    for(int j=0;j<4;j++){
      if ( p1[i][j] == '#' ) {
        if ( i + pos1.first < 0 || i + pos1.first > 3 ) {
          return;
        }
        if ( j + pos1.second < 0 || j + pos1.second > 3 ) {
          return;
        }
        grid[i+pos1.first][j+pos1.second] = '#';
      }
    }
  }
  for(int i=0;i<4;i++){
    for(int j=0;j<4;j++){
      if ( p2[i][j] == '#' ) {
        if ( i + pos2.first < 0 || i + pos2.first > 3 ) {
          return;
        }
        if ( j + pos2.second < 0 || j + pos2.second > 3 ) {
          return;
        }
        grid[i+pos2.first][j+pos2.second] = '#';
      }
    }
  }
  for(int i=0;i<4;i++){
    for(int j=0;j<4;j++){
      if ( p3[i][j] == '#' ) {
        if ( i + pos3.first < 0 || i + pos3.first > 3 ) {
          return;
        }
        if ( j + pos3.second < 0 || j + pos3.second > 3 ) {
          return;
        }
        grid[i+pos3.first][j+pos3.second] = '#';
      }
    }
  }
#ifdef DEBUG
